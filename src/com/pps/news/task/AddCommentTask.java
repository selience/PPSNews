...
public class AddCommentTask extends GenericTask {
	...
	protected Result doInBackground(String... params) {
		...
		return httpClient.doHttpPost(Constants.getCommentCreate(), 
				new BasicNameValuePair("encode", "gbk"),
				...
				new BasicNameValuePair("user_ip", ""), // 用户发布评论IP
				new BasicNameValuePair("cmd_type", "1"), // 发布评论类型 1：视频评论 2:频道评论 3：剧集 4:专题 5:特殊页面6:直播评论
				new BasicNameValuePair("source", "mobile"), // 接口调用来源 （web,pc,v，mobile）
				new BasicNameValuePair("domain", domain),
				new BasicNameValuePair("sign", UIUtil.md5ForString(domain+"vcommet"))); // 验证字符串算法(md5($domain.'vcommet'))
	}
