...
import com.pps.news.widget.SlideNavigationView.CanvasTransformer;
...
public class SlideViewBehind extends ViewGroup {

	private static final String TAG = "SlideViewAbove";

	private static final int MARGIN_THRESHOLD = 48; // dips
	private int mTouchMode = SlideNavigationView.TOUCHMODE_MARGIN;
	...
	public SlideViewBehind(Context context) {
		this(context, null);
	}

	public SlideViewBehind(Context context, AttributeSet attrs) {
		super(context, attrs);
	...
	public void setCustomViewAbove(SlideViewAbove customViewAbove) {
	...
	public void setMode(int mode) {
		if (mode == SlideNavigationView.LEFT || mode == SlideNavigationView.RIGHT) {
			if (mContent != null)
	...
	public int getMenuPage(int page) {
		...
		if (mMode == SlideNavigationView.LEFT && page > 1) {
			return 0;
		} else if (mMode == SlideNavigationView.RIGHT && page < 1) {
			return 2;
		} else {
			return page;
		}
...
//			Log.v(TAG, "behind INVISIBLE");
	...
	public boolean menuOpenTouchAllowed(View content, int currPage, float x) {
		switch (mTouchMode) {
		case SlideNavigationView.TOUCHMODE_FULLSCREEN:
			return true;
		case SlideNavigationView.TOUCHMODE_MARGIN:
